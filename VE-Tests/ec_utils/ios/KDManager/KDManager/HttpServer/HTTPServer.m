	...
	close:(BOOL)closeFileHandle
{
    ...
    [incomingRequests removeObjectForKey:[[NSNumber alloc] initWithInt:incomingFileHandle.fileDescriptor]];
}
...
- (void)stop
{
	...
	for (NSNumber * key in [incomingRequests copy])
	{
        NSFileHandle * incomingFileHandle = [incomingRequests objectForKey:key];
...
- (void)receiveIncomingConnectionNotification:(NSNotification *)notification
{
    ...
    if(incomingFileHandle)
	{
        ...
        NSValue * value = [NSValue valueWithPointer:message];
        HTTPEntry * entry = [[HTTPEntry alloc] init];
        entry.fileHandle = incomingFileHandle;
        entry.data = value;
        [incomingRequests setObject:entry forKey:[[NSNumber alloc] initWithInt:incomingFileHandle.fileDescriptor]];
...
- (void)receiveIncomingDataNotification:(NSNotification *)notification
{
    ...
    HTTPEntry * entry = [incomingRequests objectForKey:[[NSNumber alloc] initWithInt:incomingFileHandle.fileDescriptor ]];
    NSValue * value = entry.data;
    CFHTTPMessageRef incomingRequest = [value pointerValue];
	if (!incomingRequest)
	{
		[self stopReceivingForFileHandle:incomingFileHandle close:YES];
		return;
