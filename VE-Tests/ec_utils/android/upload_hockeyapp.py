...
def usage(reason):
    ...
    print "Usage: upload_hockeyapp.py <platform><Hockeyapp Token><App path><Notes>"
    assert 0
...
def update_version(version, haToken, appPath, appId):
   ...
   #     print res.json()
        ...
        res = getstatusoutput("curl --tlsv1 "\
                        "-F 'status=2' "
                        "-F 'notify=1' "
...
def upload_app(haToken, appPath, notes):
        print 'upload_app'
...
checkArguments()
upload_app(sys.argv[2], sys.argv[3], sys.argv[4])
