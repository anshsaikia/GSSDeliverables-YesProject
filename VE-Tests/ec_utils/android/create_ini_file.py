...
def usage(reason):
    if reason:
        print reason
    print "Usage: create_ini_file.py <device id> <default ini> <target new ini path> <device host> <appium port> <lab> <project> <screenShot_directory> <cmdcRegion> <vgwUtils>"
    assert 0
...
if __name__ == '__main__':
    ...
    optionParser.add_option("--subscriptions")
    ...
    if len(positional_args) > 8:
        cmdcRegion = positional_args[8]
    if len(positional_args) > 9:
        vgwUtils = positional_args[9]
    ...
    "Add CI_VARIABLES"
    if 'CI_VARIABLES' not in configParser.sections():
        configParser.add_section('CI_VARIABLES')
    configParser.set('CI_VARIABLES', 'vlan.name', lab)
    ...
    if configParser.get("APPIUM_VARS", "platformName") == 'iOS':
        ...
        if project is not None and project == 'KD':
            client_port = "5050"
        elif project is not None and "androidtv" in configParser.get("APPIUM_VARS", "appPackage"):
            client_port = "5050"
        else:
            client_port = "8080"
    ...
    if screenShot_directory is not None:
        ...
        if configParser.get("APPIUM_VARS", "platformName") != 'iOS':
            configParser.set('general', 'shotByShot', 'True')
    if vgwUtils is not None:
        configParser.set('general', 'vgwUtils', vgwUtils)
    configParser.set('general', 'verbose', 'True')
    ...
    "Add USER_VARIABLES"
    if 'USER_VARIABLES' not in configParser.sections():
        configParser.add_section('USER_VARIABLES')

    if 'user.subscriptions' not in configParser.options('USER_VARIABLES'):
    	configParser.set('USER_VARIABLES', 'user.subscriptions', named_args.subscriptions)

    if (cmdcRegion):
        configParser.set('USER_VARIABLES', 'user.cmdc_region', cmdcRegion)
