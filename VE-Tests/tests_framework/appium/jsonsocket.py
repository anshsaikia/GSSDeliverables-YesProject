...
class Server(object):
  ...
  def recv_ex(self, timeout):
    if not self.client:
      raise Exception('Cannot receive data, no client is connected')
    return _recv_timeout(self.client, timeout)
...
def _recv(socket):
  ...
  print "View: [" + view.tobytes() + "]"
  try:
    deserialized = json.loads(view.tobytes())
  except (TypeError, ValueError), e:
    raise Exception('Data received was not in JSON format %s' %e)
  return deserialized

def _recv_timeout(the_socket,timeout=2):
