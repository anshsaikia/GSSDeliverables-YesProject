...
@IHmark.O_Malka
def test_playback_skip_backward_grade_out():
    # vgw test utils must be installed for running this test
    from vgw_test_utils.settings import Settings
    ve_test = VeTestApi("test_playback_skip_backward_grade_out")
    ve_test.begin()
...
@IHmark.O_Malka
def test_playback_skip_scroll_bar_disable():
    # vgw test utils must be installed for running this test
    from vgw_test_utils.settings import Settings
    ve_test = VeTestApi("test_playback_skip_scroll_bar_disable")
    ve_test.begin()
    ...
    window_width, window_height = ve_test.milestones.getWindowSize()
    ve_test.appium.tap(window_width / 2, window_height / 2)
    element = ve_test.milestones.getElement([('id', 'playBackScrubberBar', '==')])

    ve_test.wait(30)
    time_before = ve_test.milestones.getElement([('id', 'startTime', '==')])[u'title_text']

    logging.info("Try to move ScrubberBar begining")
    ve_test.appium.tap(int(element['x_pos']), int(element['y_pos']))
